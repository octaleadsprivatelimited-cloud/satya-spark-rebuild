...
import { ArrowRight, ExternalLink, ArrowLeft, ChevronLeft, ChevronRight } from "lucide-react";
import { useEffect, useState } from "react";
import type { CatalogProduct as Product } from "@/lib/product-catalog";
import { QuoteDialog } from "@/components/QuoteDialog";
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@/components/ui/dialog";
import { loadProductGallery } from "@/lib/product-catalog";
...
export function ProductCard({
  ...
  const [cardPhotos, setCardPhotos] = useState<string[] | null>(null);
  ...
  const moveCardPhoto = async (direction: number) => {
    if (cardLoading) return;
    setCardError("");
    if (cardPhotos) {
      setCardPhotoIndex(
        (index) => (index + direction + cardPhotos.length) % Math.max(cardPhotos.length, 1),
      );
      return;
    }
    setCardLoading(true);
    try {
      const photos = getProductPhotos(await loadProductGallery(p));
      setCardPhotos(photos);
      setCardPhotoIndex(photos.length > 1 ? (direction > 0 ? 1 : photos.length - 1) : 0);
    } catch {
      setCardError("Photos unavailable. Please retry.");
    } finally {
      setCardLoading(false);
    }
  };

  const currentProduct = details?.id === activeProduct.id ? details : activeProduct;
  const photos = getProductPhotos(currentProduct);
  const photoIndex = Math.max(0, photos.indexOf(selectedImage));
  const movePhoto = (direction: number) => {
    if (photos.length)
      setSelectedImage(photos[(photoIndex + direction + photos.length) % photos.length]);
  };
  useEffect(() => {
    if (!isOpen) return;
    let cancelled = false;
    setDetails(null);
    setDetailsError("");
    loadProductGallery(activeProduct, retry > 0)
      .then((product) => {
        if (cancelled) return;
        setDetails(product);
      ...
      .catch(() => {
        if (!cancelled) setDetailsError("Photos could not be loaded. Please retry.");
      });
    return () => {
      cancelled = true;
    };
  }, [isOpen, activeProduct, retry]);
  ...
  return (
    <>
      ...
      >
        ...
        >
          <ProductImage
            product={cardPhotos?.[cardPhotoIndex] ? { ...p, image: cardPhotos[cardPhotoIndex] } : p}
            alt={`${p.name} — ${p.brand} ${p.category}`}
        ...
        <div className="flex items-center justify-between gap-1 border-y border-border px-2 py-1.5 bg-white">
          <button
            type="button"
            aria-label={`Previous photo of ${p.name}`}
            disabled={cardLoading || cardPhotos?.length === 1}
            onClick={() => void moveCardPhoto(-1)}
            className="p-1.5 disabled:opacity-30"
          >
            <ChevronLeft className="h-4 w-4" />
          </button>
          <span aria-live="polite" className="text-xs text-center">
            {cardLoading
              ? "Loading photos…"
              : cardPhotos
                ? `${cardPhotoIndex + (cardPhotos.length ? 1 : 0)} / ${cardPhotos.length} photos`
                : "Browse photos"}
          </span>
          <button
            type="button"
            aria-label={`Next photo of ${p.name}`}
            disabled={cardLoading || cardPhotos?.length === 1}
            onClick={() => void moveCardPhoto(1)}
            className="p-1.5 disabled:opacity-30"
          >
        ...
        {cardPhotos && cardPhotos.length > 1 && (
          <div
            className="flex gap-1.5 p-2 overflow-x-auto bg-white"
            aria-label={`Uploaded photos of ${p.name}`}
          >
            {cardPhotos.map((src, index) => (
              <button
                key={src}
                type="button"
                aria-label={`Show photo ${index + 1} of ${p.name}`}
                aria-pressed={index === cardPhotoIndex}
                onClick={() => setCardPhotoIndex(index)}
                className={`h-10 w-10 shrink-0 border rounded p-0.5 ${index === cardPhotoIndex ? "border-primary ring-1 ring-primary" : "border-border"}`}
              >
                <img
                  src={src}
                  alt={`Photo ${index + 1}`}
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            <QuoteDialog
              ...
              trigger={
                <button
                  type="button"
                  className="inline-flex items-center gap-1 text-sm font-normal bg-yellow-500 text-black px-3 py-1.5 hover:bg-yellow-400 cursor-pointer group/btn"
                >
                  Get a quote
            ...
            {activeTab === "details" ? (
              <div className="grid md:grid-cols-2 gap-6 md:gap-8 items-start mt-4 md:mt-0">
                <div className="flex flex-col gap-4">
                  <div className="bg-white border border-border overflow-hidden aspect-[4/3] flex items-center justify-center p-2">
                    <img
                      src={selectedImage || currentProduct.image || undefined}
                      decoding="async"
                      alt={`${currentProduct.name} — ${currentProduct.brand}`}
                  ...
                  {details && (
                    <div className="flex items-center justify-between gap-3">
                      <button
                        ...
                        onClick={() => movePhoto(-1)}
                        className="border border-border rounded p-2 disabled:opacity-30"
                      >
              ...
              <div className="space-y-4">
                <div className="w-full h-[500px] sm:h-[600px] border border-border bg-muted overflow-hidden">
                  <iframe
                    src={getEmbedUrl(currentProduct.pdf)}
                    className="w-full h-full border-0"
                    title={`${currentProduct.name} Brochure`}
                    allow="autoplay"
                  ></iframe>
                </div>
                <div className="flex justify-end">
                  <a
                    href={currentProduct.pdf}
                    target="_blank"
                    rel="noreferrer"
