    ...
    <>
      ...
      <section className="py-12 md:py-16 bg-background">
        <div className="mx-auto max-w-[1920px] px-6 md:px-16 grid md:grid-cols-[240px_1fr] lg:grid-cols-[260px_1fr] gap-10 items-start">
          ...
          <div>
            {error && (
              <div role="alert" className="mb-4 border border-border p-4 text-sm">
                {error}{" "}
                <button
                  type="button"
                  className="underline"
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
              <div className="text-sm text-muted-foreground">
                {searchQuery ? (
                  <span>
                    Found {filtered.length} {filtered.length === 1 ? "product" : "products"} for "
            ...
            {loading && products.length === 0 ? (
              <div className="grid grid-cols-2 sm:grid-cols-2 xl:grid-cols-3 gap-5">
                {[...Array(6)].map((_, i) => (
                  <div
                    key={i}
                    className="flex flex-col bg-card border border-border h-[320px] animate-pulse"
                  >
